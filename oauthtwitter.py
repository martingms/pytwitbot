...
'''
Modifications made by Martin Gammelsæter (@martingamm):
    
    Encoding utf-8
    Line 82 has been modified to 'url = req.normalized_url' from 'url = req.get_normalized_http_url()'
    Function GetFollowersOf(user_id) added, so you can get followers for other users than the logged in one.
    Function GetFollowersIds() added
    Function GetFriendsIds() added
'''
...
class OAuthApi():
    ...
    def GetFollowersIds(self, options={}):
        '''Return followers ids in a list
        
        '''
    	return self.ApiCall("followers/ids", "GET", options)
    
    	
    def GetFollowersOf(self, user_id, options={}):
        '''Get the user_id's followers id's
        
        for documentation, see:
            http://dev.twitter.com/doc/get/followers/ids
        
        function added by Martin
        '''
        options['user_id'] = user_id
        return self.ApiCall("followers/ids", "GET", options)
